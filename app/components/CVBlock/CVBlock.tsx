...
export default function CVBlock({ items }: CVBlockProps) {
  return (
    <>
      ...
      <CVBlock.Timeline>
        {items.map((item) => (
          <CVBlock.TimelineItem key={item.company} item={item} />
        ))}
      </CVBlock.Timeline>
    </>
  );
...
function TimelineItem({
  item: { company, companyAbout, duration, roles },
}: TimelineItemProps) {
  return (
    // <li after:h-[calc(100%-24px)] ...>
    <li className="relative mb-12 after:h-[calc(100%+1rem)] after:-left-[1.125rem] after:absolute after:top-8 after:w-0.5 after:bg-slate-300 after:content-normal print:after:border">
      ...
      <h3 className="text-xl font-semibold">
        {company}{" "}
        <time className="text-base ml-2 font-normal leading-normal">({duration})</time>
      </h3>
      <h4 className="italic text-base print:text-sm">{companyAbout}</h4>
      {roles.map((role) => {
        const key = [role.company || "company", role.role, role.location]
          .map((i) => i.trim())
          .filter((i) => i !== "")
          .join("-");
        const companyLabel = [role.company ?? ""]
          .map((i) => i.trim())
          .filter((i) => i !== "")
          .join(", ");
        const roleLabel = [role.role, role.location]
          .map((i) => i.trim())
          .filter((i) => i !== "")
          .join(", ");
        return (
            ...
            <div className="prose prose-ul:!my-4 prose-li:my-0 print:prose-ul:my-2 dark:text-white">
              <ul className="">
                {role.tasks.map((task) => (
                  <li
                    key={task}
                    className="marker:text-gray-600 text-black dark:text-white text-base print:text-sm"
                  >
                    {task}
                  </li>
                ))}
